#!/usr/bin/env python3
"""
Fill missing artist Tidal IDs from MusicBrainz.

Tidal has no public, unauthenticated search API, so this mirrors
fill_spotify.py's approach instead of hitting Tidal directly: MusicBrainz
aggregates external streaming links, including `https://tidal.com/artist/<id>`
URL relations (relationship type "streaming"), sourced from Wikidata and
manual edits.

The script is intentionally conservative, same as fill_spotify.py:
- it only edits files under content/artists/*/index.md;
- it skips artists that already have a non-empty socials.tidal value;
...
- it requires exactly one Tidal id on that MusicBrainz artist;
- ambiguous names, missing relations, multiple Tidal ids, and network errors
  are reported but left unchanged.

Examples:
  python3 snippets/fill_tidal.py --dry-run --limit 25
  python3 snippets/fill_tidal.py --insecure-ssl
  python3 snippets/fill_tidal.py --from-slug deicide --limit 100
"""
...
DEFAULT_CACHE_DIR = Path("/tmp/muzic_live_musicbrainz_tidal_cache")
DEFAULT_REPORT = Path("/tmp/muzic_live_artist_tidal_musicbrainz_report.tsv")
...
TIDAL_ARTIST_RE = re.compile(r"tidal\.com/(?:browse/)?artist/(\d+)")
...
def tidal_value(frontmatter: str) -> str:
    match = re.search(r"^\s{2}tidal:\s*(.*?)\s*$", frontmatter, re.M)
    return unquote_yaml_scalar(match.group(1)) if match else ""


def has_non_empty_tidal(frontmatter: str) -> bool:
    return bool(tidal_value(frontmatter).strip())
...
def load_artist_files(from_slug: str | None = None) -> list[ArtistFile]:
    ...
    for path in sorted(ARTISTS_DIR.glob("*/index.md")):
        ...
        if has_non_empty_tidal(frontmatter):
            continue
...
def tidal_ids_from_relations(lookup_data: dict[str, Any]) -> tuple[list[str], list[str]]:
    ids: set[str] = set()
    urls: list[str] = []
    for relation in lookup_data.get("relations") or []:
        url = ((relation.get("url") or {}).get("resource") or "").strip()
        match = TIDAL_ARTIST_RE.search(url)
        if match:
...
def set_tidal(text: str, tidal_id: str) -> str | None:
    split = split_frontmatter(text)
    if not split:
        return None
    ...
    lines.insert(insert_at, f'  tidal: "{tidal_id}"')
    return "---\n" + "\n".join(lines) + "\n---" + body
...
def process_artist(
    ...
    tidal_ids, tidal_urls = tidal_ids_from_relations(lookup_data)
    if len(tidal_ids) != 1:
        return (
            "skip",
            "",
            ...
            candidate.get("name", ""),
            "",
            f"tidal_ids={len(tidal_ids)}",
        )

    new_text = set_tidal(artist.text, tidal_ids[0])
    if not new_text or new_text == artist.text:
        return (
            ...
            candidate.get("id", ""),
            candidate.get("name", ""),
            tidal_urls[0] if tidal_urls else "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fill missing content/artists socials.tidal values from MusicBrainz URL relations."
    )
...
def main() -> int:
    ...
    with args.report.open("w", encoding="utf-8", newline="") as report_file:
        ...
        for index, artist in enumerate(artists, start=1):
            try:
                decision, tidal_id, mbid, mb_name, tidal_url, reason = process_artist(
                    artist=artist,
