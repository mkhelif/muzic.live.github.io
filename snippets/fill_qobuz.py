#!/usr/bin/env python3
"""
Fill missing artist Qobuz IDs from MusicBrainz.

Qobuz has no public, unauthenticated search API, so this mirrors
fill_spotify.py's approach instead of hitting Qobuz directly: MusicBrainz
aggregates external streaming/purchase links, including numeric Qobuz artist
ids embedded in `https://www.qobuz.com/<locale>/interpreter/<slug>/<id>` and
`https://open.qobuz.com/artist/<id>` URL relations (relationship types
"streaming" / "purchase for download"), sourced from Wikidata and manual
edits. Older Qobuz relations that only carry a slug (no trailing numeric id,
e.g. a "download-streaming-albums" catalogue link) are skipped since they
don't resolve to a usable id.

The script is intentionally conservative, same as fill_spotify.py:
- it only edits files under content/artists/*/index.md;
- it skips artists that already have a non-empty socials.qobuz value;
...
- it requires exactly one Qobuz id on that MusicBrainz artist;
- ambiguous names, missing relations, multiple Qobuz ids, and network errors
  are reported but left unchanged.

Examples:
  python3 snippets/fill_qobuz.py --dry-run --limit 25
  python3 snippets/fill_qobuz.py --insecure-ssl
  python3 snippets/fill_qobuz.py --from-slug deicide --limit 100
"""
...
DEFAULT_CACHE_DIR = Path("/tmp/muzic_live_musicbrainz_qobuz_cache")
DEFAULT_REPORT = Path("/tmp/muzic_live_artist_qobuz_musicbrainz_report.tsv")
...
QOBUZ_ARTIST_RE = re.compile(r"music\.amazon\.[a-z.]+/artists/([A-Za-z0-9]+)")
...
def qobuz_value(frontmatter: str) -> str:
    match = re.search(r"^\s{2}qobuz:\s*(.*?)\s*$", frontmatter, re.M)
    return unquote_yaml_scalar(match.group(1)) if match else ""


def has_non_empty_qobuz(frontmatter: str) -> bool:
    return bool(qobuz_value(frontmatter).strip())
...
def load_artist_files(from_slug: str | None = None) -> list[ArtistFile]:
    ...
    for path in sorted(ARTISTS_DIR.glob("*/index.md")):
        ...
        if has_non_empty_qobuz(frontmatter):
            continue
...
def qobuz_ids_from_relations(lookup_data: dict[str, Any]) -> tuple[list[str], list[str]]:
    ids: set[str] = set()
    urls: list[str] = []
    for relation in lookup_data.get("relations") or []:
        url = ((relation.get("url") or {}).get("resource") or "").strip()
        match = QOBUZ_ARTIST_RE.search(url)
        if match:
...
def set_qobuz(text: str, qobuz_id: str) -> str | None:
    split = split_frontmatter(text)
    if not split:
        return None
    ...
    lines.insert(insert_at, f'  qobuz: "{qobuz_id}"')
    return "---\n" + "\n".join(lines) + "\n---" + body
...
def process_artist(
    ...
    qobuz_ids, qobuz_urls = qobuz_ids_from_relations(lookup_data)
    if len(qobuz_ids) != 1:
        return (
            "skip",
            "",
            ...
            candidate.get("name", ""),
            "",
            f"qobuz_ids={len(qobuz_ids)}",
        )

    new_text = set_qobuz(artist.text, qobuz_ids[0])
    if not new_text or new_text == artist.text:
        return (
            ...
            candidate.get("id", ""),
            candidate.get("name", ""),
            qobuz_urls[0] if qobuz_urls else "",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fill missing content/artists socials.qobuz values from MusicBrainz URL relations."
    )
...
def main() -> int:
    ...
    with args.report.open("w", encoding="utf-8", newline="") as report_file:
        ...
        for index, artist in enumerate(artists, start=1):
            try:
                decision, qobuz_id, mbid, mb_name, qobuz_url, reason = process_artist(
                    artist=artist,
